      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {revenueStats && (
          <button
            ...
            className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition text-sm"
          >
            <Download className="w-4 h-4" />
            <span className="hidden sm:inline">Export revenus</span>
...
function KPICard({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-4 sm:p-6 space-y-2 sm:space-y-3">
      ...
      <div>
        ...
        {isLoading ? (
          <p className="text-lg sm:text-2xl font-bold text-gray-500">...</p>
        ) : (
          <p className={`text-lg sm:text-2xl font-bold ${colorClasses[color]}`}>{value}</p>
        )}
      </div>
    </div>
  );
